...
XML_LOG_ROOT="${XML_LOG_DIR:-${RUNNER_TEMP:-$TE_PATH/logs}/cuda}"
...
run_suite() {
    ...
    case "$suite" in
        ...
        -h | --help) usage; exit 0 ;;
        *)
            echo "Unsupported CUDA test suite: $suite" >&2
            usage >&2
            exit 2
            ;;
    esac
...
if [ "$FAILED" -ne 0 ]; then
    echo "Failed CUDA test suites: ${FAILED_SUITES[*]}" >&2
    exit 1
fi

echo "All selected CUDA test suites passed."
