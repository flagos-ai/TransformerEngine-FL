...
def skip_cuda_build() -> bool:
    """Check if CUDA build was skipped (TE_FL_SKIP_CUDA=1).
    
    This is used to conditionally skip sanity checks and CUDA-specific initialization
    when running in FL-only mode (AMD/ROCm or pure Triton backends).
    """
    return bool(int(os.getenv("TE_FL_SKIP_CUDA", "0")))


# Plugin system initialization (FL backends)
# This must happen BEFORE any CUDA library loading:
# 1. Check if we should skip CUDA build (TE_FL_SKIP_CUDA env var)
