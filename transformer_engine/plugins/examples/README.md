...
```bash
cd transformer_engine/plugins/examples
...
```python
from transformer_engine.plugins.transformer_engine_fl import OpImpl, BackendImplKind
...
|----------|-------------|--------|---------|
| `TE_FL_PREFER` | Preferred backend type (highest priority) | `default` / `vendor` / `reference` | `default` |
| `TE_FL_PREFER_VENDOR` | Prefer vendor backend (legacy, lower priority than `TE_FL_PREFER`) | `1` = prefer vendor, `0` = prefer default | `0` |
...
| Variable | Description | Example |
|----------|-------------|---------|
| `TE_FL_PER_OP` | Per-operator backend ordering | `rmsnorm_fwd=vendor:acme\|default;rope_fwd=default\|reference` |
...
```bash
# Use acme vendor for rmsnorm, default for others
export TE_FL_PER_OP="rmsnorm_fwd=vendor:acme|default"
python your_script.py
```
