...
from transformer_engine.pytorch.attention.dot_product_attention.gems_sdpa import (
...
## GemsFlashAttnFunc, missing gems.flash_attention_forward
...
## GemsSdpaAttnFunc
class GemsSdpaAttnFunc(torch.autograd.Function):
    """FusedAttention forward and backward implementation"""
    ...
    def forward(
        ...
        # pylint: disable=missing-function-docstring
        assert not fp8, "Gems Sdpa Attention do not support fp8 now"
        # add NVTX range
        nvtx_label = "transformer_engine.GemsSdpaAttnFunc.forward"
        nvtx_range_push(f"{nvtx_label}")
        ...
        is_causal = attn_mask_type == 'causal'
        gems_q = q.permute(1, 2, 0, 3) #[s, b, n_h, h] -> [b, n_h, s, h]
        gems_k = k.permute(1, 2, 0, 3)
        gems_v = v.permute(1, 2, 0, 3)
        (gems_out, gems_m) = scaled_dot_product_attention_forward(
            gems_q,
            gems_k,
            gems_v,
            attn_mask=None,
        ...
        out = gems_out.permute(2, 0, 1, 3) # [b, n_h, s, h] -> [s, b, n_h, h]
        aux_ctx_tensors = [gems_out, gems_m]
        max_logit = None

        out_ret = out
        fp8_tensors = (None, None, None, None)
        qkvo_tensors = (gems_q, gems_k, gems_v, gems_out)
    ...
    def backward(ctx, d_out, *_args):
        ...
        gems_out, gems_m = aux_ctx_tensors
        rest = [None]

        with torch.cuda.nvtx.range("GemsSdpaAttnFunc.backward"):
            # get nominal data type of dq, dk, dv
            # FP16/BF16 attention: torch.float16 or torch.bfloat16
            # FP8 attention:       torch.float16 or torch.bfloat16
            dqkv_nominal_dtype = ctx.nominal_dtype
            ...
            if not gems_q.is_contiguous():
                gems_q = gems_q.contiguous()
            if not gems_k.is_contiguous():
                gems_k = gems_k.contiguous()
            if not gems_v.is_contiguous():
                gems_v = gems_v.contiguous()
            if not gems_m.is_contiguous():
                gems_m = gems_m.contiguous()
            gems_d_out = d_out.permute(1, 2, 0, 3).contiguous() # [s, b, n_h, h] -> [b, n_h, s, h]
            gems_dq, gems_dk, gems_dv = scaled_dot_product_attention_backward(
                gems_d_out,
                gems_q,
                gems_k,
                gems_v,
            ...
            dq = gems_dq.permute(2, 0, 1, 3)
            dk = gems_dk.permute(2, 0, 1, 3)
            dv = gems_dv.permute(2, 0, 1, 3)
            rest = None
...
class GemsAttention(torch.nn.Module):
    ...
    def forward(
        ...
        if context_parallel:
            ...
            with self.attention_dropout_ctx():
                # _gems_attn_impl = GemsFlashAttnFunc
                _gems_attn_impl = GemsSdpaAttnFunc
                output = _gems_attn_impl.apply(
                    self.training,
                    max_seqlen_q,
                    max_seqlen_kv,
                    cu_seqlens_q,
