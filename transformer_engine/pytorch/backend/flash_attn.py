...
class GemsFlashAttention(torch.nn.Module):
    ...
    def forward(
        ...
        ), "GemsAttention only supports FP16 and BF16 data types, or Float8Tensors."
        assert (
            query_layer.is_cuda and key_layer.is_cuda and value_layer.is_cuda
        ), "GemsAttention only supports CUDA tensors."
        assert (
            qkv_layout in QKVLayouts
        ), f"GemsAttention does not support qkv_layout = {qkv_layout}!"
        ...
        assert not context_parallel, "GemsAttention do not support context parallel now"
