...
_musa_libs_loaded = False


def _ensure_musa_libs():
...
def _get_tex():
    ...
    return transformer_engine_musa_torch


class MUSABackend(TEFLBackendBase):
    @staticmethod
    def check_available() -> bool:
        return _check_musa_available()
    ...
    def get_attention_backend(self, attention_params=None):
        """
        MUSA backend uses the default attention backend selection logic.
        This allows hardware-specific checks and optimizations for MUSA devices.
        Returns:
        ...
        from transformer_engine_musa.pytorch.attention import (
            get_attention_backend as _original_get_attention_backend,
        )

        return _original_get_attention_backend(attention_params)
